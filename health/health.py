...
def classifier(s,dec_rate):
	...
	train_inputfile = open("train_data","r")
	train_lablefile = open("train_labels","r")
	train_sensefile = open("train_sens","r")

	test_inputfile = open("test_data","r")
	test_lablefile = open("test_labels","r")
	test_sensefile = open("test_sens","r")
	...
	for i in train_sensefile.readlines():
		train_sens_attrs.append(int(i.strip("\n")))

	for i in test_sensefile.readlines():
		test_sens_attrs.append(int(i.strip("\n")))
	...
	X_train, X_test, vocab_size = data_preprocessing_v2(whole_data,train_text,test_text,max_len=71)
	...
	X_placeholder = tf.placeholder(tf.int32, [None, 71])
	Y_placeholder = tf.placeholder(tf.float32, [None, DIM_OUTPUT])
	keep_prob = tf.placeholder(tf.float32)
	mask = tf.placeholder(tf.float32,[None,71])
	...
	with tf.Session() as sess:
		...
		print(fairlearn_metrics.demographic_parity_difference(Y_test, test_pred, sensitive_features=test_sens_attrs))
		original_fairness_results =fairlearn_metrics.demographic_parity_difference(Y_test, test_pred, sensitive_features=test_sens_attrs)
		...
		for i in range(71):
			...
			print(fairlearn_metrics.demographic_parity_difference(Y_test, test_pred, sensitive_features=test_sens_attrs))
			fairness_results.append(fairlearn_metrics.demographic_parity_difference(Y_test, test_pred, sensitive_features=test_sens_attrs))
